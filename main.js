...
document.querySelectorAll('.deconstructed-card').forEach(card => {
  card.addEventListener('mousemove', e => {
    ...
    const x = (e.clientX - rect.left) / rect.width;
    const y = (e.clientY - rect.top)  / rect.height;
    const rotX = (y - 0.5) *  8;  // angle en ° selon Y
    const rotY = (x - 0.5) * -8;  // angle en ° selon X
    ...
    card.querySelectorAll('.card-layer').forEach((layer, i) => {
      const depth = 20 * (i+1);
      const moveX = (x - 0.5) *  10 * (i+1);
      const moveY = (y - 0.5) *  10 * (i+1);
      layer.style.transform = `
  ...
  card.addEventListener('mouseleave', () => {
    card.style.transform = '';
    card.querySelectorAll('.card-layer').forEach(l => l.style.transform = '');
  });
});
...
document.querySelectorAll('.deconstructed-card').forEach(card => {
  ...
  card.addEventListener('mousemove', e => {
    ...
    const rotX = (y - 0.5) *  8;  // inclinaison selon Y
    const rotY = (x - 0.5) * -8;  // inclinaison selon X
    card.style.transform = 
      `perspective(1200px) rotateX(${rotX}deg) rotateY(${rotY}deg)`;

    // Décalage progressif de chaque calque
    card.querySelectorAll('.card-layer').forEach((layer, i) => {
      const depth    = 20 * (i+1);                  // distance Z
      const offsetX  = (x - 0.5) * 10 * (i+1);
      const offsetY  = (y - 0.5) * 10 * (i+1);
      layer.style.transform =
        `translate3d(${offsetX}px,${offsetY}px,${depth}px)`;
    });
  });
